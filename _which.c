 ...
 * la variable d'environnement PATH. Pour chaque répertoire, elle
 * concatène le chemin avec la commande donnée, puis vérifie si ce
 * fichier existe à l'aide de la fonction stat().
 * Si un chemin valide est trouvé, la fonction retourne une chaîne de
 * caractères contenant le chemin complet de la commande.
...
char *_which(const char *cmd)
{
	...
	path_copy = strdup(path); /*cpy du path,pas de modif dans path d'origine*/
	token = strtok(path_copy, ":"); /*on dit quoi découper(path_copy) & où(:)*/
...
/*avec les strings token et cmd*/
		...
		free(full_path);/*on free la mémoire pour traiter une autre cmd*/
		token = strtok(NULL, ":");/*on cut le path (voilà pq copie du path)*/
	}
